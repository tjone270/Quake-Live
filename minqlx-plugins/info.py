# Created by Thomas Jones and vks (on QuakeNet IRC) on 21/01/2016 - thomas@tomtecsolutions.com
# info.py, a plugin for minqlx to show a list of aliases and IPs that a player has used to connect to the server. Reads from the Redis database.
# This plugin is released to everyone, for any purpose. It comes with no warranty, no guarantee it works, it's released AS IS.
# You can modify everything, except for lines 1-4 and the !tomtec_versions code. They're there to indicate I whacked this together originally. Please make it better :D

import minqlx
import minqlx.database

class info(minqlx.Plugin):
    database = minqlx.database.Redis

    def __init__(self):
    	self.add_command("info", self.cmd_info, 3, usage="<id>")
    	self.add_command("tomtec_versions", self.cmd_showversion)

    	self.plugin_version = "1.1"

    def cmd_info(self, player, msg, channel):
        if len(msg) < 2:
            return minqlx.RET_USAGE

        try:
            steam_id = self.player(int(msg[1])).steam_id
            player_name = self.player(int(msg[1])).name
        except:
            channel.reply("^1Invalid Client ID.^7 Enter a valid client ID to see a list of aliases and IP addresses they've used on this server.")
            return

        key = "minqlx:players:{}:ips".format(steam_id)
        out = list(self.db.smembers(key))
        channel.reply("{}^7 has played on this server from the following IP addresses:".format(player_name))
        channel.reply(" ^4*^7  {}".format("   \n ^4*^7  ".join(out)))

        key = "minqlx:players:{}".format(steam_id)
        out = list(self.db.lrange(key, 0, -1))
        channel.reply("{}^7 has played on this server under the following names:".format(player_name))
        channel.reply(" ^4*^7  {}".format("   \n ^4*^7  ".join(out)))

    def cmd_showversion(self, player, msg, channel):
        channel.reply("^4ips.py^7 - version {}, created by Thomas Jones on 21/01/2016.".format(self.plugin_version))
